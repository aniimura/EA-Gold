...
//|  Source of truth : D:\EA_Gold_TradingView\FxTrade_202608\strategies\voldip.py
//|  Generated       : 2026-08-29 20:03:31
//|
...
#define STRAT_NAME        "VolDip"
#define WARMUP_BARS       57
#define RATES_COUNT       121
#define MAX_HOLD_BARS     48
#define MIN_BARS_BETWEEN  24
...
double Ind_std50(int s)
  {
   ...
   return(MathSqrt(acc / 49.0));
  }

//--- bb_width = Expr('4.0 * std50 / sma50')
double Ind_bb_width(int s)
  {
   return(((4.0 * Ind_std50((s) + 0)) / Ind_sma50((s) + 0)));
  }

//--- slope = Expr('(sma50 - sma50[5]) / atr')
double Ind_slope(int s)
  {
   return(((Ind_sma50((s) + 0) - Ind_sma50((s) + 5)) / Ind_atr((s) + 0)));
  }
...
int OnInit()
  {
   ...
   if(InpWriteTrades)
     {
      g_fh_trades = FileOpen("fxtrade_VolDip_trades.csv",
   ...
   if(InpWriteBars)
     {
      g_fh_bars = FileOpen("fxtrade_VolDip_bars.csv",
                           FILE_WRITE|FILE_CSV|FILE_ANSI|FILE_COMMON, ',');
      if(g_fh_bars != INVALID_HANDLE)
         FileWrite(g_fh_bars, "time", "atr", "sma50", "std50", "bb_width", "slope", "sig_long", "sig_short", "spread");
     }
...
void OnNewBar()
  {
   ...
   if(!g_in_pos && !closed_this_bar &&
     ...
     {
      if(((Ind_bb_width(0) > 0.0158) && (Ind_slope(0) < (-0.79))))
         TryEnter(true);
      else if(false)
         TryEnter(false);
     }
   ...
   if(InpWriteBars && g_fh_bars != INVALID_HANDLE && g_bars_written < InpBarsCsvMax)
     {
      ...
      FileWrite(g_fh_bars, TimeToString(g_rates[1].time, TIME_DATE|TIME_SECONDS),
                          ...
                          DoubleToString(Ind_sma50(0), 10),
                          DoubleToString(Ind_std50(0), 10),
                          DoubleToString(Ind_bb_width(0), 10),
                          DoubleToString(Ind_slope(0), 10),
                          (((Ind_bb_width(0) > 0.0158) && (Ind_slope(0) < (-0.79))) ? "1" : "0"),
                          (false ? "1" : "0"),
