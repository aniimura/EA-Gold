...
//|  Source of truth : D:\EA_Gold_TradingView\FxTrade_202608\strategies\rsi_revert.py
//|  Generated       : 2026-08-29 20:00:11
//|
...
#define STRAT_NAME        "RsiRevert"
#define WARMUP_BARS       602
#define RATES_COUNT       666
#define MAX_HOLD_BARS     72
#define MIN_BARS_BETWEEN  12
#define EXPECT_TF         PERIOD_H1
...
double Ind_atr(int s)
  {
   double sum = 0.0;
   for(int j = 0; j < 14; j++)
      sum += MathMax(g_rates[(s + j) + 1].high - g_rates[(s + j) + 1].low, MathMax(MathAbs(g_rates[(s + j) + 1].high - g_rates[(s + j) + 2].close), MathAbs(g_rates[(s + j) + 1].low - g_rates[(s + j) + 2].close)));
   return(sum / 14.0);
  }

//--- rsi = RSI(14)
double Ind_rsi(int s)
  {
   double a = 1.0 / 14.0;
   double g = 0.0, l = 0.0;
   for(int j = 0; j < 14; j++)
     {
      g += MathMax(g_rates[(s + 139 - j) + 1].close - g_rates[((s + 139 - j) + 1) + 1].close, 0.0);
      l += MathMax(g_rates[((s + 139 - j) + 1) + 1].close - g_rates[(s + 139 - j) + 1].close, 0.0);
     }
   g /= 14.0; l /= 14.0;
   for(int k = 125; k >= 0; k--)
     {
      g = a * (MathMax(g_rates[(s + k) + 1].close - g_rates[((s + k) + 1) + 1].close, 0.0)) + (1.0 - a) * g;
      l = a * (MathMax(g_rates[((s + k) + 1) + 1].close - g_rates[(s + k) + 1].close, 0.0)) + (1.0 - a) * l;
     }
...
//--- ema100 = EMA(100)
double Ind_ema100(int s)
  {
   double a = 2.0 / (100.0 + 1.0);
   double seed = 0.0;
   for(int j = 0; j < 100; j++)
      seed += g_rates[(s + 599 - j) + 1].close;
   seed /= 100.0;
   double e = seed;
   for(int k = 499; k >= 0; k--)
      e = a * (g_rates[(s + k) + 1].close) + (1.0 - a) * e;
   return(e);
  }

//--- hi20 = Highest(20)
double Ind_hi20(int s)
  {
   double v = g_rates[(s) + 1].high;
   for(int j = 1; j < 20; j++)
      v = MathMax(v, g_rates[(s + j) + 1].high);
   return(v);
  }

//--- stretch = Expr('(ema100 - close) / atr')
double Ind_stretch(int s)
  {
   return(((Ind_ema100((s) + 0) - g_rates[(s) + 0 + 1].close) / Ind_atr((s) + 0)));
  }
...
int OnInit()
  {
   ...
   if(_Symbol != "GBPUSD")
      PrintFormat("[%s] WARNING chart symbol %s != spec symbol %s",
                  STRAT_NAME, _Symbol, "GBPUSD");

   if(InpWriteTrades)
     {
      g_fh_trades = FileOpen("fxtrade_RsiRevert_trades.csv",
   ...
   if(InpWriteBars)
     {
      g_fh_bars = FileOpen("fxtrade_RsiRevert_bars.csv",
                           FILE_WRITE|FILE_CSV|FILE_ANSI|FILE_COMMON, ',');
      if(g_fh_bars != INVALID_HANDLE)
         FileWrite(g_fh_bars, "time", "atr", "rsi", "ema100", "hi20", "stretch", "sig_long", "sig_short", "spread");
     }
...
void TryEnter(bool is_long)
  {
   ...
   sl_dist = 2.0 * atr_v;
   tp_dist = 3.0 * atr_v;
   if(sl_dist <= 0.0 && tp_dist <= 0.0 && MAX_HOLD_BARS <= 0 && !true)
      return;
...
void OnNewBar()
  {
   ...
   if(g_in_pos)
     {
      ...
      bool exit_sig = g_dir_long ? ((Ind_rsi(0) > 60.0)) : (false);
   ...
   if(!g_in_pos && !closed_this_bar &&
     ...
     {
      if(((Ind_rsi(0) < 30.0) && (g_rates[0 + 1].close < Ind_ema100(0)) && (Ind_stretch(0) > 0.5)))
         TryEnter(true);
      else if(false)
         TryEnter(false);
     }
   ...
   if(InpWriteBars && g_fh_bars != INVALID_HANDLE && g_bars_written < InpBarsCsvMax)
     {
      ...
      FileWrite(g_fh_bars, TimeToString(g_rates[1].time, TIME_DATE|TIME_SECONDS),
                          ...
                          DoubleToString(Ind_rsi(0), 10),
                          DoubleToString(Ind_ema100(0), 10),
                          DoubleToString(Ind_hi20(0), 10),
                          DoubleToString(Ind_stretch(0), 10),
                          (((Ind_rsi(0) < 30.0) && (g_rates[0 + 1].close < Ind_ema100(0)) && (Ind_stretch(0) > 0.5)) ? "1" : "0"),
                          (false ? "1" : "0"),
